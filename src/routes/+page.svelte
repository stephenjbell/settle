<script>
	...
	function checkCards() {
		...
		props.forEach((prop) => {
			...
			if (
				cardChecker[0][prop] === cardChecker[1][prop] &&
				cardChecker[1][prop] === cardChecker[2][prop]
			) {
				...
				cardChecker[0][prop] !== cardChecker[1][prop] &&
				cardChecker[1][prop] !== cardChecker[2][prop] &&
				cardChecker[0][prop] !== cardChecker[2][prop]
			) {
				...
				if (i !== j) {
					...
					for (let k = 0; k < playSpots.length; k++) {
						if (i !== k && j !== k) {
							...
							if (
								allThreeCardsExist &&
								checkSpotsSet(playSpots[i].card, playSpots[j].card, playSpots[k].card)
							) {
				...
				<div class="cards">
					{#each { length: 3 } as _, i}
						<div class="cardslot">
							{#if cardChecker[i]}
								<div
									class="card quantity{cardChecker[i].quantity} {cardChecker[i].color} {cardChecker[
										i
									].shading} {cardChecker[i].shape}"
								>
									<div class="front">
										{#each Array(cardChecker[i].quantity) as _}
											<img
												src="/images/shapes/{cardChecker[i].color}-{cardChecker[i]
													.shading}-{cardChecker[i].shape}.svg"
												alt=""
											/>
										{/each}
									</div>
	...
	<div class="cardtable" style="--cardWidth:{cardWidth}px;">
		<div class="displaycards">
			{#each Object.keys(displayCards) as cardName}
				<div
					class="card {displayCards[cardName].location} [ quantity{displayCards[cardName]
						.quantity} {displayCards[cardName].color} {displayCards[cardName]
						.shading} {displayCards[cardName].shape} ] {displayCards[cardName].now.rotZ === 90
						? 'vertical'
						: 'horizontal'}"
					style="
				...
				>
					...
					<div class="front">
						{#each Array(displayCards[cardName].quantity) as _}
							<img
								src="/images/shapes/{displayCards[cardName].color}-{displayCards[cardName]
									.shading}-{displayCards[cardName].shape}.svg"
								alt=""
							/>
						{/each}
					</div>
			...
			{#each { length: settings.numberOfPlayers } as _, i}
				...
				>
					<div class="cardpile" style="width:{cardWidth}px;">
						...
						{:else}
							<div class="buttontext">Click to play</div>
						{/if}
					</div>
					<div class="key">
				...
				{#each { length: 4 } as num, i}
					<label>
						<input
							...
							bind:group={settings.numberOfPlayers}
							value={i + 1}
						/>
						{i + 1}
					</label>
